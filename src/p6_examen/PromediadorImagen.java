...
//	}
	...
	public void vorazExamenSinBalanceo(int n_tries) {
		int count = n_tries;
		double zncc = -1;
...
//		printSol();
	...
	public void vorazExamenConBalanceo(int n_tries, int max_unbalancing) {
		int count = n_tries;
		double zncc = -1;
...
//		printSol();
	...
	public void backtrackingExamenSinBalanceo() {
		...
		backtrackingExamenRec(0);

//		printSol();
	}

	private void backtrackingExamenRec(int level) {
		...
		if (level == sol.length) { // Caso base
			...
			for (int i = 0; i < 4; i++) {
				this.counter++;
				sol[level] = i;
				backtrackingExamenRec(level + 1);
			}
		}

	}

	public void backtrackingExamenConBalanceo(int max_unbalancing) {
		...
		backtrackingExamenBalanceoRec(0, max_unbalancing);

//		printSol();
	}

	private void backtrackingExamenBalanceoRec(int level, int max_unbalancing) {
		...
		if (level == sol.length) {
			...
			if (Math.abs(numUnos - numDoses) <= max_unbalancing) {
				for (int i = 0; i < 4; i++) {
					...
					sol[level] = i;
					backtrackingExamenBalanceoRec(level + 1, max_unbalancing);
					if (i == 1) {
						numUnos--;
					} else if (i == 2) {
						numDoses--;
